...
import com.valorant.registerProgress.from.RegisterProgressForm;
...
public class RegisterProgressController {
    ...
    @PostMapping("api/v1/admin/registerProgress")
    public void updateRegisterProgress(@RequestBody List<RegisterProgressForm> registerProgressFormList) {

        List<RegisterProgress> registerProgressList = new ArrayList<>();

        registerProgressFormList.forEach(registerProgressForm -> {
            String area = ESAPI.encoder().decodeFromJSON(registerProgressForm.getArea());
            String count = ESAPI.encoder().decodeFromJSON(registerProgressForm.getCount());

            RegisterProgress registerProgress = registerProgressService.findByArea(area);
            registerProgress.setCount(Long.parseLong(count));
            registerProgressList.add(registerProgress);
        });

        registerProgressService.saveAll(registerProgressList);
    }
}
